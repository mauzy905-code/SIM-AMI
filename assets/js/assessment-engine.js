    ...
    function createAssessmentModule(deps) {
        ...
        function renderRekapButton(patient) {
            ...
            const available = registry.listAll().filter(function (s) {
                ...
                if (s.id === 'triase_ugd') return isUgdRow && isPerawat;
                if (s.id === 'pediatrik_awal') return isPediatrikRow && isNs;
                if (s.id === 'rawat_jalan_dewasa') return !isPediatrikRow && isNs;
                return isNs;
            });
        ...
        function handleRekapButtonClick(btnEl) {
            ...
            const isTriaseUgd = String(schemaId || '').trim().toLowerCase() === 'triase_ugd';
            const isNs = (typeof window.isNurseStationRole === 'function') ? Boolean(window.isNurseStationRole()) : false;
            const isPerawat = (typeof window.isPerawatRole === 'function') ? Boolean(window.isPerawatRole()) : false;
            const allowed = isTriaseUgd ? isPerawat : isNs;
            if (!allowed) {
                ...
                if (isTriaseUgd) {
                    alert('[AKSES DITOLAK]\n\nTombol Triase UGD pada halaman Rekap Pasien HANYA dapat dibuka oleh akun PERAWAT SAJA.\n\nRole Anda saat ini: ' + curRole + '\n\nSilakan login sebagai Perawat UGD.');
                } else {
                    alert('[AKSES DITOLAK]\n\nTombol asesmen (Rawat Jalan / Pediatrik) pada halaman Rekap Pasien HANYA dapat dibuka oleh akun PETUGAS NURSE STATION.\n\nRole Anda saat ini: ' + curRole + '\n\nSilakan login sebagai Nurse Station.');
                }
                return;
            }
